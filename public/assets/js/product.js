...
(function () {
  ...
  if (videoContainer) {
    if (p.youtubeUrl) {
      let videoId = null;
      const youtuBeMatch = p.youtubeUrl.match(/youtu\.be\/([a-zA-Z0-9_-]+)/);
      const youtubeMatch = p.youtubeUrl.match(/youtube\.com\/watch\?v=([a-zA-Z0-9_-]+)/);
  ...
  // Keep add-to-cart button attributes in sync with current selection state.
  function wireCartButton(p) {
    ...
    function syncButtonAttrs() {
      ...
      btn.setAttribute('data-item-price', Number(p.price ?? p.base_price ?? 0));
    }
  ...
  const card = p => `
    <article class="rel-card">
      <a class="rel-card__img" href="${esc(productUrl(p))}">
        <img src="${esc(p.image || '/images/placeholder.png')}"
