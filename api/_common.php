...
function bb_admin_password(): string {
  ...
  if (is_file($localPassFile)) {
    $raw = @file_get_contents($localPassFile);
...
function bb_products_path(): string {
  // Allow explicit override in hosting environments.
  $override = getenv('PRODUCTS_JSON_PATH');
  if (is_string($override) && trim($override) !== '') {
    return trim($override);
  }
  if (!empty($_SERVER['PRODUCTS_JSON_PATH'])) {
    return trim((string) $_SERVER['PRODUCTS_JSON_PATH']);
  }

  // Local repo-first fallback for this project.
  $repoDataPath = realpath(__DIR__ . '/../src/_data/products.json');
  if ($repoDataPath && is_file($repoDataPath)) {
    return $repoDataPath;
  }

  // Cloudways / classic hosting fallback.
...
function bb_products_file_is_wrapped(string $path): bool {
  if (!is_file($path)) {
    // New files in this repo should default to wrapped shape.
    return (bool) preg_match('#[\\\/]src[\\\/]_data[\\\/]products\.json$#i', $path);
  }
  $raw = @file_get_contents($path);
  $decoded = $raw ? json_decode($raw, true) : null;
  return is_array($decoded) && array_key_exists('products', $decoded);
}
...
function bb_validate_products(array $incoming): array {
  ...
  foreach ($incoming as $idx => $p) {
    ...
    $rawPrice = $p['price'] ?? '';
    $price = bb_parse_price($rawPrice);
    if ($price === null) {
      // Keep seeded catalogs editable even when upstream sheets omit prices.
      $price = '0.00';
    }
    ...
    if ($image === '') $image = '/assets/img/placeholder.png';
    ...
    if (!empty($badge)) $out['badge'] = $badge;
    else unset($out['badge']);
    if (!empty($badges)) $out['badges'] = $badges;
    else unset($out['badges']);
